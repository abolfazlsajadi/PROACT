...
\end{danger}
\vfill
{\small Generated from the maintained public host sources and documentation,
with firmware/RTL behavior documented from the companion design package.}
\end{titlepage}
...
bash tools/setup_env.sh --with all   # one-time: creates/reuses project .venv
...
\begin{notebox}
This public source checkout does not include the controller/Sw-RV firmware
sources or built images. The commands below apply in the companion design
package. For bench use, build there or supply matching \code{.vmem} files
separately and select them explicitly in the host tools.
\end{notebox}
\begin{lstlisting}[style=sh]
...
\begin{center}\includegraphics[width=\textwidth]{gui_overview.png}\end{center}
...
\begin{center}\includegraphics[width=\textwidth]{gui_chipwhisperer.png}\end{center}
...
\begin{center}\includegraphics[width=\textwidth]{gui_cpa.png}\end{center}
\textbf{CPA analysis:} the offline correlation power-analysis attack, on its own
page. Browse to a separately supplied \code{.npz}/\code{.h5}; leaving
\emph{Capture} empty checks the conventional
\code{datasets/<core>\_reference.npz} path, but this public checkout contains no
trace dataset. The legacy GUI runner also requires the companion
\code{examples/cpa\_*.py} scripts. Pick the leakage
...
\begin{center}\includegraphics[width=\textwidth]{gui_registers.png}\end{center}
...
\begin{center}\includegraphics[width=\textwidth]{gui_memory.png}\end{center}
...
\begin{center}\includegraphics[width=\textwidth]{gui_selfcheck.png}\end{center}
...
the controller command server (conventionally
\code{Software/Controller/main.vmem}) --- is supplied separately in this public
source release. It is the on-chip program that understands all the GUI's commands;
the Programming panel has a one-click button to select that conventional path when
the file is present, or the operator can browse to another matching image.
...
2026-08-07). The fabricated \textbf{ASIC is also screened}: the same A--Z check
reports 16 pass / 0 fail / 0 skip on the CW308 target board (2026-09-11).
Every claim below states exactly how it was verified.
\end{notebox}
\begin{center}\rowcolors{2}{rowalt}{white}
...
On the fabricated ASIC & \textbf{hardware} --- A--Z screen reports 16 pass / 0 fail / 0 skip on the CW308 target board, including clock lock + trace capture \\
\bottomrule
\end{longtable}\end{center}
