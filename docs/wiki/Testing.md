...
> The unified A–Z self-check **runs on real hardware and passes**: the PROACT FPGA build on a real CW305 board, driven over the real MCP2210/MCP2200 bench links. The GUI-driven run reported **16 pass / 0 fail / 0 skip**, including a ChipWhisperer Husky clock lock and a real trace capture; the same check from the CLI with no scope attached is **14 pass / 0 fail / 1 skip** (the capture step SKIPs). The **fabricated ASIC is bench-verified**: a die on the CW308 board loads firmware over SPI, answers the UART, reproduces the AES1/AES2 and Sw-RV known-answer vectors on silicon, locks the Husky clock and trigger, and sustains long unattended trace-capture runs. The 16/16 A–Z score above is the CW305 FPGA build; the same A–Z check remains the screening procedure for further chips. **Windows and macOS are untested.** Every other "PASS" on this page comes from software builds, an RTL simulator, or host-side offline checks; the labels further down identify the level of each result.
