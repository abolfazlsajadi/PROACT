...
>
> **Verification status:** these numbers are **RTL-verified and gate-sim-verified**, and the map has now been exercised end-to-end **on the real CW305 FPGA**: the unified A–Z self-check (`proact_host/fullcheck.py`) passes 100% — UART, AES1/AES2 encrypt + decrypt, ASCON/Xoodyak on-chip encrypt KAT, timer, control register, PRNG, and Sw-RV. **The fabricated ASIC has not been screened yet** (the same self-check screens it unchanged). The ASIC and FPGA share this map **byte-for-byte** — there is no address divergence between them.
